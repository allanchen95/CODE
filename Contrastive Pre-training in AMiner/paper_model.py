...
class bertEmbeddingLayer(nn.Module):
    def __init__(self):
        ...
        self.Encoder = nn.Sequential(
            nn.Linear(configs["bert_size"], configs["hidden_size"])
        )       
...
class matchingModel(nn.Module):
    ...
    def get_intersect_matrix(self, paper_embed, author_embed):
        ...
        paper_embed = paper_embed.view(paper_shape[0], 1, configs["hidden_size"])
        paper_embed = paper_embed.repeat(1, author_shape[0], 1).view(paper_shape[0]*author_shape[0], configs["hidden_size"])
        author_embed = author_embed.repeat(paper_shape[0], 1)
        # print(paper_embed.size())
        # print(author_embed.size())
        sim_vec = torch.tanh(F.pairwise_distance(paper_embed, author_embed))
