...
class process_matching_data:
    ...
    def _load_raw_data(self):
        ...
        # test_data
        with open(paper_data_dir + "paper_data/test_author_filter_cna_5_unass.json", 'r') as files:
        ...
        self.test_namepaper2pid = self.construct_test_paper_data(self.unass_data, self.valid_authors)
    ...
    def get_test_author_encoder(self, raw_paper_id, author_id, neg_author_id_list, author_name, author_dict):
        ...
        for neg_author_id in neg_author_id_list:
            ...
            neg_author_list = list(neg_author_set)
            random.seed(configs["seed"])
            # sample_papers_c = random.sample(neg_author_list, configs["min_papers_each_author"])
            sample_papers_c = random.sample(neg_author_list, min(500, len(neg_author_list)))
            # print("neg:",len(sample_papers_c))
            # ct.append(len(sample_papers_c))
            input_ids_list_c = []
            ...
            # print("neg:", len(input_ids_list_c))
            ct.append(len(input_ids_list_c))
        # print("pos: {} aver neg: {}".format(len(input_ids_list_b), np.mean(np.array(ct))))
