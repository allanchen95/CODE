...
if __name__ == "__main__":
    ...
    for epoch_num in range(20):
        ...
        s_time = time.time()
        batch_loss = []
        for batch_num in range(len(batch_train_data)):
            batch_data = batch_train_data[batch_num]
            ...
            for ins_num in range(len(batch_data)):
                ...
                pos_score = matching_model(a_author_embeddings, b_author_embeddings)
                # pos_loss, pos_score = contrastive_model(a_author_embeddings, b_author_embeddings, 1)
                # instance_mean_loss.append(pos_loss)
                # pos_score = torch.cosine_similarity(a_author_embeddings, b_author_embeddings)
                ...
                instance_mean_loss = torch.mean(instance_mean_loss)
                # print("matching_loss: ", matching_batch_loss)

                instance_mean_loss = instance_mean_loss / len(batch_data)
                instance_mean_loss.backward()
                # optimizer.step()
                # optimizer.zero_grad()           
                # exit()

                # batch_loss.append(instance_mean_loss.item())
                epoch_loss.append(instance_mean_loss.item())
                scheduler_loss.append(instance_mean_loss.item())

            # if((ins_num + 1) % configs["local_accum_step"] == 0):
                # mean_batch_loss = np.mean(batch_loss)
            optimizer.step()
            # my_lr_scheduler.step()
            optimizer.zero_grad()
            if((batch_num + 1) %  int(total_batch_num/3) == 0):
                # my_lr_scheduler.step()
                print("current_step: {} loss: {:.3f} cost: {:.3f}".format((batch_num+ 1), np.sum(scheduler_loss), time.time() - s_time))
                scheduler_loss = []
                # test_loss = []
                # matching_score = []
                ratio_top_k =  test_cluster_model(embedding_model,matching_model, test_data)
                embedding_model.train()
                matching_model.train()
                if(ratio_top_k[0] > max_hits):
                    ...
                    torch.save(state, file_name + "model_" + str(epoch_num) + "_" + str((batch_num + 1)))
                # matching_model.train()  
                # embedding_model.train()
                # shared_encoder.train()
        print("Epoch: {} loss: {:.3f} cost: {:.3f}".format(epoch_num, np.sum(epoch_loss), time.time() - s_time))
